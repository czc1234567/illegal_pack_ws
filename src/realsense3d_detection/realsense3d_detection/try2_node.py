...
from std_msgs.msg import String, Bool  # 【新增】引入 Bool 类型
...
class RealSense3DDetectionNode(Node):
    def __init__(self):
        ...
        self.get_logger().info("🚀 系统启动：检测 + TTS + 永久标记 + WebJSON推送 + 导航联动")
    ...
    def publish_web_json(self, cv_image, marker_array, header):
        try:
            ...
            self.get_logger().info(f"📤 Web JSON 推送: {len(markers_data)} 个标记")
        except Exception as e:
            self.get_logger().error(f"JSON发布失败: {e}")
    ...
    def processing_loop(self):
        while rclpy.ok():
            ...
            if cv2.waitKey(1) & 0xFF == ord('q'): rclpy.shutdown(); break
